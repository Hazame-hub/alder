...
func Playbook(tasks string) string {
	var b strings.Builder
	b.Grow(len(tasks) + playbookPreambleLen)
	_ = WritePlaybook(&b, tasks)
	return b.String()
}

// playbookPreambleLen is roughly the fixed header below, so the builder above
// sizes itself once instead of doubling its way up through a large document.
const playbookPreambleLen = 512

// WritePlaybook is Playbook, written straight into w.
//
// The indenting used to go through strings.Split, which on a subtree export is
// a slice header for every line of a thirty-megabyte document, and then built
// the whole indented copy in memory before any of it was sent. Walking the
// lines with strings.Cut holds one line at a time and hands the rest to the
// connection as it goes.
func WritePlaybook(w io.Writer, tasks string) error {
	if _, err := io.WriteString(w, playbookHeader); err != nil {
		return err
	}
	rest := strings.TrimRight(tasks, "\n")
	for {
		...
		if line == "" {
			...
			// Three writes rather than one concatenation: the concatenation
			// allocated a copy of every line in the document.
...
const playbookHeader = "---\n" +
	"# Generated by Alder. Review before running.\n" +
	"#\n" +
	"# The connection variables are left unset on purpose: a bind password does\n" +
	"# not belong in a generated file. Supply them from a vault, the command\n" +
	"# line, or the environment.\n" +
	"- name: Apply directory changes\n" +
	"  hosts: localhost\n" +
	"  gather_facts: false\n" +
	"  vars:\n" +
	"    ldap_server_uri: \"ldaps://directory.example.test\"\n" +
